...
class DualIntentEntityTransformer(pl.LightningModule):
    ...
    def configure_optimizers(self):
        intent_optimizer = optim.Adam(self.parameters(), lr=self.intent_optimizer_lr)
        entity_optimizer = optim.Adam(self.parameters(), lr=self.entity_optimizer_lr)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        intent_pred = intent_pred.cpu()
        intent_idx = intent_idx.cpu()

        intent_acc = get_accuracy(intent_idx, intent_pred.argmax(1))[0]
        entity_acc = get_token_accuracy(
            entity_idx, entity_pred.argmax(2), ignore_index=self.dataset.pad_token_id
        )[0]
