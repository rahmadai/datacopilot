...
def show_intent_report(
    dataset, pl_module, tokenizer, file_name=None, output_dir=None, cuda=True
):
...
def show_entity_report(
    ...
    for batch in tqdm(dataloader, desc="load entity dataset"):
        ...
        for i in range(entity_idx.shape[0]):
            decode_original = decoder.process(
                input_ids[i].cpu().numpy(), entity_idx[i].numpy(), token[i]
            )
            decode_pred = decoder.process(
                input_ids[i].cpu().numpy(), entity_indices[i].numpy(), token[i]
            )
    ...
    report = show_entity_metrics(
        pred=preds, label=targets, file_name=file_name, output_dir=output_dir
    )
